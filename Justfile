...
clone_vm node_id:
	ssh macpro qm clone {{template}} 3000{{ node_id }} --pool onedr0p-k3s ;
	ssh macpro qm set 3000{{ node_id }} -efidisk0 zssd:1,pre-enrolled-keys=0 -boot order=scsi0 -net0 "virtio,bridge=vmbr0,tag={{tag}}" -ipconfig0 "ip=192.168.30.3{{ node_id }}/24,gw=192.168.30.1,ip6=auto" -sshkey /root/pve-macpro.pub -ciuser "{{ciuser}}" -cipassword "{{cipassword}}"

name_vm node_id name:
	ssh macpro qm set 3000{{node_id}} --name {{name}}

start_vm node_id:
	ssh macpro qm start 3000{{node_id}}

stop_vm node_id:
	ssh macpro qm stop 3000{{node_id}}

destroy_vm node_id:
  -ssh macpro qm stop 3000{{node_id}}
  ssh macpro qm destroy 3000{{node_id}}
